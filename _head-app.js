/**
 * Full Experience ΓÇö view-only 3D bookshelf (baked library).
...
/** Desktop reference size ΓÇö small viewports keep this scale and pan instead. */
...
/** View-only: always false ΓÇö never call setEditing(true). */
let editing = false;
...
async function boot() {
  ...
  centerView();
  buildScene();
  // Mobile often lays out after the first paint ΓÇö refit once the stage has size.
...
function sanitizeState(next = state) {
  ...
  next.units.forEach((unit) => {
    ...
    // Size + plane bounds only ΓÇö never rewrite from screen projection.
    clampUnitInPlane(unit);
    enforceShelfShares(unit);
  });
  return next;
...
function shelfCountLimits(_unit) {
  // Count slider is always 1ΓÇªMAX; size only blocks shrinking the case, not adding back.
...
function minShareForUnit(unit) {
  ...
  const hPx = caseHeightPx(unit);
  if (hPx <= 1) return 1 / n;
  // Never demand more than an equal split when the case is too short for n├ùMIN_SHELF_PX
...
/* ΓöÇΓöÇ Geometry / snap / collision ΓöÇΓöÇ */

/**
 * Hard size floors/ceilings only.
 * Does NOT apply the pixel shelf floor ΓÇö that is viewport-dependent and would
 ...
 * Stable across window resize ΓÇö stored positions are never rewritten from
 * viewport projection.
 */
...
function snapUnitResize(unit, edge) {
  ...
  // Do not call clampUnitBounds here ΓÇö it translates the case. Resize finalize
...
function buildUnitDom(unit, unitIndex) {
  ...
  if (editing) {
    // Flat front pick plane ΓÇö CSS 3D faces don't receive clicks reliably.
...
function syncInspector() {
  ...
  if (!unit) {
    weightOut.textContent = 'ΓÇö';
    booksOut.textContent = 'ΓÇö';
    countOut.textContent = 'ΓÇö';
  ...
  if (!shelf) {
    weightOut.textContent = 'ΓÇö';
    booksOut.textContent = 'ΓÇö';
    editHint.textContent = 'Selection lost ΓÇö click an object again.';
    return;
  }
...
function addAdjacent(dir) {
  ...
  if (!slot) {
    editHint.textContent = 'No free space in that direction ΓÇö resize or move cases first.';
    return;
  }
...
/* Edit interaction listeners omitted ΓÇö VIEW_ONLY bookshelf viewer. */
bindSearchUi();
boot();
